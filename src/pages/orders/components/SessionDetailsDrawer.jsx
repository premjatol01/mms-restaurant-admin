...
export default function SessionDetailsDrawer({ session, onClose, onMarkPayment, onCloseSession }) {
  const { orders, getTableName, getOrdersBySession } = useOrdersStore();
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative bg-surface w-full max-w-md h-full flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-theme">
          <div>
            <h2 className="text-lg font-semibold text-theme">{getTableName(session.tableId)}</h2>
            <p className="text-sm text-secondary">{session.sessionNumber}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="font-medium text-theme flex items-center gap-2">
              <Users size={16} /> Orders in this session
            </h3>
                ...
                <div key={order.id} className="flex justify-between items-center p-3 bg-primary-light/10 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-secondary">
                      {order.customer?.mobile || "Customer"}
                    </p>
                  </div>
          ...
          <div className="space-y-3">
            <h3 className="font-medium text-theme flex items-center gap-2">
              <DollarSign size={16} /> Consolidated Bill
            </h3>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                session.paymentStatus === "pending" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"
              }`}>
        ...
        <div className="px-6 py-4 border-t border-theme space-y-2">
          {session.paymentStatus === "pending" ? (
            <button
              onClick={handleMarkPaymentClick}
            ...
            <button
              ...
              className="w-full py-2.5 bg-primary hover:bg-primary-light text-white rounded-lg font-medium transition-colors"
            >
              Close Session
            </button>
          )}
